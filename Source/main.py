...
from utils import generate_samples, get_loader, save_model, my_decode, gen_masked_ids
...
def train_generator_PG(train_dataloader, gen:Generator, dis:Discriminator, rollout:Rollout, pg_loss:PGLoss, nllloss, gen_optimizer, dis_optimizer, args):
    ...
    for step, examples in enumerate(bar, 1):
        ...
        logits = discriminator(
                ...
                code_ids=code_ids,
                convert=True)
        dis_loss = nllloss(F.log_softmax(logits, dim=-1), torch.ones((code_ids.size(0)),dtype=torch.long).to(gen.device))
        dis_loss.backward()
        with torch.no_grad():
            _, _, cur_generated_examples = rollout.get_reward_simplified_v3(examples, args.n_rollout, dis) # shape: batchsize * sequence length
        targets = torch.tensor([ex.code for ex in examples], dtype=torch.long)\
            .to(args.device).view(-1)
        generated_examples.extend(cur_generated_examples)
        original_examples.extend(examples)

        gnerated_code_ids = torch.tensor(
            [ex.code for ex in cur_generated_examples], dtype=torch.long
            ).to(gen.device)
        logits = discriminator(
                comment_ids=doc_ids,
                code_ids=gnerated_code_ids,
                convert=True)
        dis_loss = nllloss(F.log_softmax(logits, dim=-1), torch.zeros((code_ids.size(0)),dtype=torch.long).to(gen.device))
        dis_loss.backward()
        dis_optimizer.step()
        dis_optimizer.zero_grad()
        
        rewards, output, cur_generated_examples = rollout.get_reward_simplified_v3(examples, args.n_rollout, dis) # shape: batchsize * sequence length
...
def eval_generator_bleu(generator, dataloader, args, output_path=None):
    ...
    with torch.no_grad():
        for idx, examples in enumerate(bar, 1):
            ...
            gnerated_code_ids = samples.cpu().numpy().tolist()
            cur_generated_examples = deepcopy(examples)
            for i in range(len(output.sequences)):
                cur_generated_examples[i].code = [x for x in gnerated_code_ids[i] if x not in special_ids]
            generated_examples.extend(cur_generated_examples)
...
def train_discriminator(dis:Discriminator, gen, criterion, optimizer, epochs, 
    ...
    if not os.path.exists(positive_file):
        positive_file = args.train_filename
    ...
    for epoch in range(epochs):
        ...
        for step, examples in enumerate(bar, 1):
            ...
            doc_ids = torch.tensor([ex.doc + [gen.tokenizer.pad_token_id]*(args.max_source_length-len(ex.doc)) for ex in examples]).to(args.device)
            code_ids = torch.tensor([ex.code + [gen.tokenizer.pad_token_id]*(args.max_target_length-len(ex.code)) for ex in examples]).to(args.device)
...
if __name__ == '__main__':
    ...
    discriminator, tokenizer_d = build_or_load_gen_model(args, 'discriminator', args.discriminator_name_or_path)
    discriminator.tokenizer_g = tokenizer_g
    ...
    dis_optimizer = optim.Adagrad(params=discriminator.parameters(), lr=args.dis_lr)
    ...
    if args.do_train:
        for i in range(args.rounds):
            ...
            if args.do_eval:
                ...
                torch.save([gen_eval_dataloader.dataset.feats[i] for i in gen_eval_dataloader.sampler], positive_file)
                print("gen eval loss: {:.5f}, eval bleu: {:.5f}, eval CodeBLEU: {:.5f}\n"
                    .format(gen_loss, gen_bleu, gen_code_bleu))
    ...
    if args.do_test:
        ...
        negative_file = args.test_filename.replace(".jsonl", tokenizer_type + "_generated.exps")
        if args.save_result is True:
            negative_file = args.test_filename.replace(".jsonl", \
                tokenizer_type + os.path.basename(os.path.dirname(args.load_generator_path)) + "_generated.jsonl")
        _, _, gen_eval_dataloader = get_loader(args.test_filename, args, tokenizer_g, pool, 
                                               ...
                                               save_purified_file=True, samplenum=args.evaluate_sample_size) 
        torch.save([gen_eval_dataloader.dataset.feats[i] for i in gen_eval_dataloader.sampler], positive_file)
        gen_bleu, code_bleu, crystalbleu = eval_generator_bleu(generator, gen_eval_dataloader, \
            args, output_path=negative_file)
